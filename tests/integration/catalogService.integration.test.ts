...
import {
  ...
  type TableHandler,
} from "@/tests/support/supabaseMock";
...
describe("catalogService Supabase integration", () => {
  ...
  it("uses static catalog when the supabaseCatalog flag is disabled", async () => {
    ...
    const { fetchCatalog, shouldUseSupabaseCatalog } = await import("@/services/catalogService");
  ...
  it("maps marketplace_listings_view rows into ServiceProduct records", async () => {
    ...
    const { fetchCatalogFromSupabase } = await import("@/services/catalogService");
  ...
  it("fetches a paginated catalog page from product_search_index", async () => {
    ...
    const { fetchCatalogPage } = await import("@/services/catalogService");
