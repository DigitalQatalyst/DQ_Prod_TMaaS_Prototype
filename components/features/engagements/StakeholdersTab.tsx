...
const STAKEHOLDER_TYPES: {
...
}[] = [
  {
    ...
    description: "Executive accountable for project.",
    minEngagement: "Quarterly",
  },
  {
    value: "Project Owner",
    label: "Project Owner",
    description: "Business owner.",
    minEngagement: "Monthly",
  },
  {
    value: "Project Manager",
    label: "Project Manager",
    description: "Day-to-day counterpart.",
    minEngagement: "Weekly",
  },
  {
    value: "Project Team",
    label: "Project Team",
    description: "Operational contributors.",
    minEngagement: "-",
  },
  {
    value: "External",
    label: "External",
    description: "Other departments outside primary delivery scope.",
    minEngagement: "-",
  },
];
...
const getInfluenceBadgeClass = (level: string) => {
  switch (level) {
    case "High":
      return "bg-navy-950 text-white border-navy-950";
    case "Medium":
      return "bg-navy-200 text-navy-900 border-navy-300";
    case "Low":
      return "bg-slate-100 text-slate-600 border-slate-200";
    default:
      return "bg-slate-100 text-slate-600 border-slate-200";
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {STAKEHOLDER_TYPES.map((t) => (
                ...
                >
                  <span
                    className={`mt-0.5 inline-block h-2.5 w-2.5 rounded-full shrink-0 ${form.type === t.value ? "bg-orange-500" : "bg-slate-300"}`}
                  />
                  <span>
...
export const StakeholdersTab = () => {
  ...
  const handleStatusChange = (id: string, newStatus: string) => {
    setStakeholders((prev) =>
      prev.map((sh) => (sh.id === id ? { ...sh, engagementStatus: newStatus } : sh))
    );
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Registry of key client and internal stakeholders to drive commitment metrics.
            </p>
          </div>
          <Button
            size="sm"
            onClick={handleAddStakeholder}
            className="gap-2 bg-green-600 hover:bg-green-700 text-white rounded-xl shadow-sm"
          >
                ...
                stakeholders.map((sh) => (
                  <TableRow key={sh.id} className="hover:bg-slate-50/50 group/sh">
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-[10px] font-semibold whitespace-nowrap ${getTypeBadgeClass(sh.type)}`}
                      >
                        {sh.type || "-"}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-[10px] font-semibold ${getInfluenceBadgeClass(sh.influence)}`}
                      >
                        {sh.influence}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-[10px] font-semibold ${getCommitmentBadgeClass(sh.commitment)}`}
                      >
                        {sh.commitment}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        {ENGAGEMENT_STATUSES.map((s) => (
                          <option key={s} value={s} className="bg-white text-navy-950">
                            {s}
                          </option>
                        ))}
                      </select>
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-3">
          {STAKEHOLDER_TYPES.map((t) => (
            <div
              key={t.value}
              className={`flex items-start gap-2 rounded-xl border px-3 py-2.5 bg-white ${getTypeBadgeClass(t.value).replace("text-", "border-").split(" ")[0]}`}
            >
              <Badge
                variant="outline"
                className={`text-[10px] font-semibold shrink-0 mt-0.5 ${getTypeBadgeClass(t.value)}`}
              >
                {t.label}
              </Badge>
