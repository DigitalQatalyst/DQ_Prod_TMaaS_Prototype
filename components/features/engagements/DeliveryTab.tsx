...
import {
  mockEngagement,
  mockKRIs as initialKRIs,
  mockMilestones as initialMilestones,
  mockDeliverables as initialDeliverables,
...
export const DeliveryTab = () => {
  ...
  const calculatedMilestones = useMemo(() => {
    return milestones.map((m) => {
      ...
      const allClosed =
        processedDels.length > 0 &&
        processedDels.every((d) => d.status === "Closed" || d.calculatedProgress === 100);
  ...
  const handleAddDeliverable = () => {
    const targetMilestone = (milestones.length > 0 ? milestones[0] : undefined) ?? {
      id: 0,
      name: "Unassigned",
    };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              The strategic outcomes defining successful delivery.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={handleAddKRI}
              className="gap-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl shadow-sm"
            >
          ...
          <CardContent className="pt-6 grid gap-4 md:grid-cols-3">
            {kris.length === 0 ? (
              ...
              kris.map((kri) => (
                <div
                  key={kri.id}
                  className="group/kri p-4 rounded-xl border border-border flex flex-col justify-between hover:border-orange-200 transition-colors relative"
                >
                  <Button
                    variant="ghost"
                    size="icon"
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-2 pr-6">
                      <Badge
                        variant="outline"
                        className="text-[10px] uppercase font-bold text-navy-700 border-navy-200 bg-white"
                      >
                        {kri.id}
                      </Badge>
                      <Badge
                  ...
                  <div className="mt-2 pt-4 border-t border-border">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {calculatedMilestones.filter((m) => (m.outcomeMapping || []).includes(kri.id))
                        .length === 0 && (
      ...
      <section className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              High-level operational schedule mapped to strategic outcomes.
            </p>
          </div>
          <Button
            size="sm"
                    ...
                    <TableRow key={milestone.id} className="hover:bg-slate-50/50 group/milestone">
                      <TableCell className="py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <Badge
                            variant="outline"
                            className={`${c.bg} ${c.text} border-current/20 font-mono px-2 py-0.5 rounded-md`}
                          >
                            MS0{index + 1}
                          </Badge>
                        ...
                        <Badge
                          ...
                          className={
                            milestone.health === "On Track"
                              ? "bg-green-50 text-green-700 border-green-200"
                              : milestone.health === "At Risk"
                                ? "bg-amber-50 text-amber-700 border-amber-200"
                                : milestone.health === "Delayed"
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1.5 w-full max-w-[140px]">
                          ...
                          <Progress
                            value={milestone.calculatedProgress}
                            className="h-1.5 bg-gray-100"
                            indicatorClassName={
                              milestone.status === "Completed" ? "bg-green-500" : "bg-navy-950"
                            }
                          />
                        </div>
                      </TableCell>
                        ...
                        <div className="flex flex-col gap-1 text-[11px]">
                          ...
                          <div className="flex justify-between gap-4">
                            ...
                            <span className="font-medium text-navy-950">
                              {milestone.originalContractDate}
                            </span>
                          </div>
                          <div className="flex justify-between gap-4">
                      ...
                      <TableCell>
                        <Button
                          ...
                          className="h-8 w-8 text-gray-400 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover/milestone:opacity-100 transition-opacity"
                        >
                          <Trash2 size={14} />
                        </Button>
                      </TableCell>
      ...
      <section className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Execution artifacts and their associated external task dependencies.
            </p>
          </div>
          <Button
            size="sm"
        ...
        <div className="space-y-6">
          {calculatedMilestones.length === 0 ||
          calculatedMilestones.every((m) => m.processedDels.length === 0) ? (
              ...
              return (
                <div key={`del-group-${milestone.id}`} className="space-y-4">
                  {milestone.processedDels.map((del) => (
                    <Card
                      key={del.id}
                      className="border-navy-100 shadow-sm overflow-hidden group/del"
                    >
                      <div className="bg-white p-6">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-start mb-4 gap-4">
                          <div>
                            <Badge
                              variant="outline"
                              className="mb-3 bg-slate-50 text-slate-700 border-slate-200"
                            >
                            ...
                            <div className="flex items-center gap-4 text-xs">
                              <span className="text-gray-500">
                                Owner:{" "}
                                <span className="font-medium text-navy-950">{del.owner}</span>
                              </span>
                              <span className="text-gray-500">•</span>
                              <span className="text-gray-500">
                                Progress:{" "}
                                <span className="font-medium text-navy-950">
                                  {del.calculatedProgress}%
                                </span>
                              </span>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className={`whitespace-nowrap ${deliverableStatusBadgeClass(del.status)}`}
                            >
                              {del.status}
                            </Badge>
                            <Button
                              ...
                              className="h-8 w-8 text-gray-400 hover:text-red-500 opacity-0 group-hover/del:opacity-100 transition-opacity"
                            >
                              <Trash2 size={14} />
                            </Button>
                          </div>
                        ...
                        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 relative">
                          <div className="flex justify-between items-center mb-3">
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                              Mapped External Tasks
                            </p>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleAddTask(del.id)}
                              className="h-6 text-[10px] px-2 text-navy-600 hover:text-navy-950 hover:bg-navy-50"
                            >
                          ...
                          <div className="grid gap-2">
                            {del.tasks.length === 0 ? (
                              <div className="text-center p-4 text-xs text-gray-400 border border-dashed border-gray-200 rounded-lg bg-white">
                                No tasks mapped yet.
                              </div>
                            ) : (
                              del.tasks.map((task) => (
                                <div
                                  key={task.id}
                                  className="group/task flex flex-col sm:flex-row sm:items-center justify-between bg-white border border-border rounded-lg p-3 text-sm hover:border-slate-300 transition-colors gap-3"
                                >
                                  <div className="flex items-center gap-3">
                                    <Badge
                                      variant="outline"
                                      className="bg-slate-100 text-slate-700 font-mono text-[10px] border-slate-200 shadow-sm"
                                    >
                                      {task.id}
                                    </Badge>
                                  ...
                                  <div className="flex items-center gap-4 sm:w-1/2 justify-end">
                                    ...
                                    <div className="flex items-center gap-2 w-24">
                                      <Progress
                                        ...
                                        indicatorClassName={
                                          task.status === "Done" ? "bg-green-500" : "bg-navy-950"
                                        }
                                      />
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="text-gray-400 hover:text-navy-950 h-6 w-6 flex items-center justify-center rounded hover:bg-navy-50"
                                      >
                                        <ExternalLink size={14} />
                                      </a>
                                      <button
                                        onClick={() => handleDeleteTask(del.id, task.id)}
                                        className="text-gray-400 hover:text-red-500 h-6 w-6 flex items-center justify-center rounded hover:bg-red-50 opacity-0 group-hover/task:opacity-100 transition-opacity"
                                      >
                                        <Trash2 size={14} />
                                      </button>
                                    </div>
