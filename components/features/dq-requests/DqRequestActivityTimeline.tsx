...
import { formatTimelineEntryForDq } from "@/lib/requests/timelineAudience";
import { formatRequestDateTime } from "@/lib/requests/format";
import { cn } from "@/lib/utils";

interface DqRequestActivityTimelineProps {
  entries: RequestTimelineEntry[];
}

export function DqRequestActivityTimeline({ entries }: DqRequestActivityTimelineProps) {
  ...
  return (
    <div className="space-y-0">
      {sorted.map((entry, index) => {
        const { title, body } = formatTimelineEntryForDq(entry);

        return (
          <div key={entry.id} className="relative flex gap-3 pb-6 last:pb-0">
            {index < sorted.length - 1 ? (
              <span
                className="absolute left-[7px] top-4 h-[calc(100%-8px)] w-px bg-[var(--color-border-subtle)]"
                aria-hidden
              />
            ) : null}
            <span
