...
const DEFAULT_RETURN_TO = "/dashboard/dq/queue";

const ENTRA_ERROR_MESSAGES: Record<string, string> = {
  entra: "Microsoft sign-in failed. Please try again.",
  entra_not_configured: "Staff sign-in is not configured for this environment.",
  ...
  staff_only: "This sign-in is for authorised delivery team accounts only.",
};
...
export default function DqSignInPageClient() {
  ...
  const handleStubSignIn = async () => {
    ...
    try {
      const res = await fetch("/api/auth/stub-session", {
        ...
        body: JSON.stringify({ audience: "internal" }),
      });
      if (!res.ok) {
    ...
    >
      <div className="relative w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-balance text-2xl font-semibold tracking-tight text-dq-navy sm:text-3xl">
            Delivery workspace sign in
          </h1>
          <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-gray-600">
            Access the request queue, assignments, and internal delivery tools.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-[0_24px_60px_-20px_rgba(3,15,53,0.12)] sm:p-9">
          ...
          <p className="mt-1.5 text-sm text-gray-600">
            Use your DigitalQatalyst Microsoft account.
          </p>
          ...
          <button
            ...
            data-testid="dq-sign-in-microsoft"
          >
            <MicrosoftIcon />
          ...
          {showDevStub ? (
            <>
              ...
              <button
                ...
                data-testid="dq-sign-in-dev-stub"
              >
                {stubLoading ? "Signing in…" : "Continue with dev staff session"}
              </button>
        ...
        <p className="mt-6 text-center text-sm text-gray-600">
          ...
          <Link href="/sign-in" className="font-medium text-dq-navy underline decoration-dq-navy/30 underline-offset-2 hover:text-dq-orange">
            Customer sign in
          </Link>
        </p>
      </div>
